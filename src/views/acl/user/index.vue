<template>
    <el-card style="height: 80px;">
        <el-form :inline="true" :model="keywordMou" class="form" ref="formInputRef" :rules="rulesInput">
            <el-form-item label="用户名:" prop="keyword">
                <el-input placeholder="请你输入搜索用户名" v-model="keywordMou.keyword"></el-input>
            </el-form-item>
            <el-form-item>
                <el-button type="primary" size="default" :disabled="keywordMou.keyword ? false : true"
...
// 获取搜索框组件实例
let formInputRef = ref();

//收集用户输入的关键字
let keywordMou = reactive<any>({
    keyword: ''
});
...
const getHasUser = async (pager = 1) => {
    //收集当前页码
    pageNo.value = pager;
    let res = await api.getUserInfo(pageNo.value, pageSize.value, keywordMou.keyword);
...
const save = async () => {
    ...
    let res: any = await api.getUserInfoAdd(userParams);
    console.log('添加或修改-------------------------', res);
...
const setRole = async (row: any) => {
    //存储已有的用户信息
    fenpeiJue.userId = row.id;
    // Object.assign(userParams, row);
    drawer1.value = true;
    // //获取全部的职位的数据与当前用户已有的职位的数据
    let res = await api.getUserInfoToAssign(row.id);
    console.log('获取角色管理数据---------------------', res);
...
const confirmClick = async () => {
    //收集参数
    fenpeiJue.roleIdList = userRole.value.map(ele => ele.id);
    
    // //分配用户的职位
    let res = await api.getUserInfoDoAssignRole(fenpeiJue);
    console.log('修改用户管理-----------------------', res);
...
const deleteUser = async (userId: number) => {
    let result: any = await api.getUserInfoDelete(userId);
...
const deleteSelectUser = async () => {
    ...
    //批量删除的请求
    let result: any = await api.getUserInfoPiDelete(idsList);
...
const rulesInput = {
    //用户名字
    keyword: [
        { required: true, validator: validatorInput, trigger: 'blur' }
    ],
}

//搜索按钮的回调
const search = async () => {
    await formInputRef.value.validate();
